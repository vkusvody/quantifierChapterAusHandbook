...
nominal affixes (\ref{ex:manytimes}), and even verbal roots ((\ref{ex:valways}); see also Pintupi (PN: Wati; \textbf{cite!}) and Yugambeh (PN: Bandjalangic; \textbf{cite!})). Although most temporal quantifiers are A-quantifiers, we notice that they are often morphologically derived from D-quantifiers. This is in line with the observations in \citet{gil93} and \citet{keenanpaperno17ov}.
...
\begin{exe}
  ...
  In 4/24 languages, %a construction with
the noun meaning `arm' (in Yir Yoront, `finger') combined with a D-quantifier % is used
yields a `times'-adverbial construction, as in (\ref{ex:2arm}). % Vanya, do you have any idea why this might be? 
...
Finally, in Waluwara (PN: Ngarna) we find an example of the causative/verbaliser -\textit{\charis ma} attaching to the numeral \textit{\charis kutja} `two,' resulting in the verb \textit{\charis kutjama} `to do something twice' (\citealt[113]{breen71}).%breen71:113
