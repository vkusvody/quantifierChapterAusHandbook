...
nominal affixes (e.g., Kalkatungu -\textit{ŋujan} `(x many) times'; see \ref{ex:manytimes} below), and even verbal roots (in Burarra (\ref{ex:valways}; also \ref{indetclass1}), Pintupi and Yugambeh). Although most TQs are A-quantifiers, we notice that they are often derived from D-quantifiers. This is in line with the observations in \citealt{gil93,keenanpaperno17ov}. %@ Decide whether to use > of / notation for person portmanteaux
