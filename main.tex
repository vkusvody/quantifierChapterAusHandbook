...
English \textit{every} and \textit{all}. However, 
this generalization is hard to state definitively due to the fact that person and number agreement marking and plural nominal marking tend to be optional in Australian languages. As a result, it can be difficult to diagnose whether a given quantifier is modifying a singular or a  plural noun. For now, we note simply that we have not found any universal force quantifiers that specifically select for singular nouns (while plural noun selection is found, e.g., in Kunbarlang, where it is not possible to have morphosyntactically singular construction modified by the universal quantifier). %We show a (lack of) such a contrast in (\ref{}).
na-kudji-kudji ka-warre
Class.I-one-one 3SG.NonFut-move.NonPast
