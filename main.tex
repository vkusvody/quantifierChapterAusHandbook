...
Relevant properties that could help decide between the two options include (un)usual word order of the negation marker (e.g.\ in Kunbarlang, the negative particle typically immediately precedes the verb, but in clauses with such indefinites it precedes the Wh-word) or optionality/obligatoriness of the negative particle (e.g.\ if the negative particle maybe omitted, because the verb sufficiently encodes the negation semantics, this may be considered optional negative concord rather than a series of negative indefinites).
...
\begin{exe}
  ...
  \begin{xlist}
    ...
    \glt `I didn't get any fish.'
    \ex\label{ex:nswide}\gll \textbf{waaɻi} ŋan-tani \textbf{ŋucica}\\
    nothing \Fsg.\Sarg$>$\Anim-spear.\Pcon{} fish\\
    \glt `I didn't spear a fish (one in particular).'
  \end{xlist}
\end{exe}

Quantifier expressions, in particular the ones with existential force, are often found to scope under negation. A small number of languages are described as having codified expressions including negation and quantifiers, e.g.\ the Mangarayi expression in (\ref{mangarayiquantneg1}), where the negated meaning is that of value judgement:
\begin{exe}
  \ex {\sc Mangarayi (nPN)} \hfill \pgcitep{merlan89}{37--38} \label{mangarayiquantneg1}
...
\begin{exe}
  ...
  \begin{xlist}
    \ex\label{ex:kudjinar} Ngunda ki-kala \textbf{na}-\textbf{kudji} (nayi) barbung.\\
    ...
    \ex\label{ex:kudjiamb} Nayi \textbf{na}-\textbf{kudji} barbung ngurnda ki-kala.\\
...
We take these findings to indicate an uncovered richness of data to explore in the course of the future first-hand work on Australian languages. However, field work on these concepts is notoriously difficult and requires carefully constructed scenarios, essentially with illustrations, to ensure the correct understanding of the truth conditions of every example by both the informant and the linguist.
