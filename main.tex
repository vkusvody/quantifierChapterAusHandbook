...
A number of volumes have shed light on the diversity of quantificational systems cross-linguistically (\citealt{bachetal95}, \citealt{qclp08}, \citealt{keenanpaperno12,keenanpaperno17}). However, with the exception of a relatively small number of publications (see especially, \citealt{laughren81}, \citealt{bittnerhale95}, \citealt{evans95}, \citealt{alpher01}, \citealt{bowernzentz12}, \citealt{bowler17}), the quantificational systems of Australian languages remain relatively under-studied. This chapter aims to make some progress towards filling this gap. In this chapter, we give a typological overview of quantificational expressions in Australia based on data from 125 languages.
...
We refer to this distinction to describe some of the morphosyntactic properties of quantificational expressions in Australian languages. We find that basic, set-describing quantifiers (i.e., translational equivalents of English \textit{all, many}, and so on) are frequently realized as nouns. We diagnose a lexical item as a noun by its ability to host case marking and/or trigger agreement marking, as in (\ref{allerg})--(\ref{agrmarking1}),\footnote{The abbreviations used in examples are: \printglossary[style=inline,type=\leipzigtype]} or if it appears within noun phrases.\footnote{Many Australian languages are described as lacking adjectives as a lexical category distinguished from nouns with respect to their morphosyntactic properties (\citealt[67--68]{dixon02}, \citealt[237--8]{nordlinger14}, \citealt[\S3.1]{louagie19}).} As such, D-quantifiers are widespread in Australian languages.
...
In addition to these D-quantifiers, the languages in our survey also have A-quantifiers. These are used to express quantificational concepts through verbal modifiers such as free adverbs (\ref{aquant2}), preverbs/coverbs (\ref{aquant3}), and verbal affixes (\ref{aquant1}).
...
In sum: to the best of our knowledge, all Australian languages have both D- and A-quantifiers, with D-quantifiers doing most of the quantificational work. This observation is interesting in the context of \cite{introqnl}'s typological generalization that while no languages lack A-quantifiers, some languages have been found to lack D-quantifiers. Our Australian data accords with their generalization in that all of the languages in our sample have A-quantifiers. However, we have not found any Australian languages to lack D-quantifiers---in fact that make heavy use of D-quantifiers in expressing quantificational concepts.
...
We also observe that there are clear historical links between numerals and expressions for `several' in many of the languages in our sample. This includes synchronic polysemies (as in (\ref{fewex0})--(\ref{fewex3})) as well as historical relationships between `several' and numerals (as in Warlpiri \textit{wirrkardu} (\ref{warlpiriseveral1})). This is documented across both Pama-Nyungan and non-Pama-Nyungan languages, suggesting that it is widespread across Australia.
...
We thank our audiences at the 9th European Australianists Workshop, the Australian Languages Workshop 2018, and at UCLA for their generous feedback on this project. We also thank our UCLA research assistants, Nick Curleo and Ryan Smick, for their help collecting data. %Finally, we thank Claire Bowern and Barry Alpher for 
