...
nominal affixes (\ref{ex:manytimes}), and even verbal roots ((\ref{ex:valways}); see also Pintupi (PN: Wati; \pgcitealt{hansenhansen77}{148}) and Yugambeh (PN: Bandjalangic; \pgcitealt{sharpe98}{171})). Although most temporal quantifiers are A-quantifiers, we notice that they are often morphologically derived from D-quantifiers. This is in line with the observations in \citet{gil93} and \citet{keenanpaperno17ov}.
\begin{exe}
...
Languages that exhibit this polysemy are Djinang, Kunbarlang, Mawng and Yir Yoront. This polysemy does not correlate with a weak distinction between adjectives and adverbs in a language. For instance, in Kunbarlang, adjectives and adverbs are distinct categories.
...
The other two major patterns are also derivational. 5/24 languages derive the `times'- adverbial from a D-quantifier by a non-specialized affix, such as limitative affix or a case marker, as in  (\ref{ex:dattimes}).
\begin{exe}
...
Finally, in Waluwara (PN: Ngarna) we find an example of the causative/verbaliser -\textit{\charis ma} attaching to the numeral \textit{\charis kutja} `two,' resulting in the verb \textit{\charis kutjama} `to do something twice' \pgcitealp{breen71}{113}.
...
In this chapter, we provided an overview of quantificational expressions in 125 Australian languages. We showed that Australian languages have equivalent expressions for all of the commonly described quantifiers. 
